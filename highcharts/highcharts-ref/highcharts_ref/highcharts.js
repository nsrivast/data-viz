/******************************************************************************
Name:    Highcharts JS
Version: 0.1 (November 23 2008)
Config: 
Author:  Torstein Hønsi
Support: Not yet

Licence:
Highcharts JS is licensed under a Creative Commons Attribution-NonCommercial 2.5
...
var userAgent = navigator.userAgent,
	...
	//hasSvgTransparentBackgroundBug = isWebKit && parseFloat(/Safari\/([\d.]+)/.exec(userAgent)[1]) >= 528, 
...
if (window.MooTools) {
	...
	function animate(el, params, options){
		/*var layer = $(el);
	 	layer.set('morph', {
	...
	// TODO: fix animate, add getJSON (and rename both)
	...
	function animate(el, params, options){	
		jQuery(el).animate(params, options);
	}
	function getJSON(url, callback) {
		return jQuery.get(url, null, callback);
	}
...
var defaultOptions = {
	...
	legend: {
		...
		itemHoverStyle: { // TODO: docs
			color: '#000'
		},
		itemHiddenStyle: { // TODO: docs
			color: '#CCC'
		},
		symbolWidth: 30,
		symbolPadding: 5
...
function createElement (tag, attribs, styles, parent, nopad) {
	...
	if (attribs) setProperties(el, attribs);
...
function setProperties (obj, props) {
	for (var x in props) obj[x] = props[x];
};

function setStyles (el, styles) {
	//for (var x in styles) el.style[x] = styles[x];
	if (isIE && typeof styles.opacity != 'undefined') 
		styles.filter = 'alpha(opacity='+ (styles.opacity * 100) +')';
	setProperties(el.style, styles);
};
function numberFormat (number, decimals, decPoint, thousandsSep) {
    ...
    var i = parseInt(n = mathAbs(+n || 0).toFixed(c)) + "", j = (j = i.length) > 3 ? j % 3 : 0;
...
function dateFormat(format, timestamp, capitalize) {
	// TODO: add more keys from strftime
	function pad (number) {
	...
	format = format.
		replace('%a', lang.weekdays[date.getUTCDay()].substr(0, 3)).
		replace('%A', lang.weekdays[date.getUTCDay()]).
		replace('%b', lang.months[date.getUTCMonth()].substr(0, 3)).
		replace('%B', lang.months[date.getUTCMonth()]).
		replace('%e', date.getUTCDate()).
		replace('%H', pad(date.getUTCHours())).
		replace('%M', pad(date.getUTCMinutes())).
		replace('%S', pad(date.getUTCSeconds())).
		replace('%y', date.getUTCFullYear().toString().substr(2, 2)).
...
Layer.prototype = {
	...
	getSvg: function() {
		if (!this.svgObject) {
			var layer = this,
				...
				width = layer.width,
				height = layer.height;
			this.svg = isIE ? '' : '<?xml version="1.0" encoding="utf-8"?>'+
			...
			if (isIE) {
		        ...
		        if (!document.namespaces["g_vml_"]) {
					...
					// setup default css
					document.createStyleSheet().cssText = "g_vml_\\:*{behavior:url(#default#VML)}";
		        }
	...
	drawRect: function(x, y, w, h, color, width, radius, fill, shadow, image) {
		...
		if (fill) {
			...
			ctx.fill();
			// draw path again
			if (window.G_vmlCanvasManager) drawPath();
		}
		if (width) {
			ctx.strokeStyle = color;
			ctx.lineWidth = width;
	...
	addText: function(str, x, y, style, rotation, align) {
		if (str || str === 0) {
			...
			var layer = this,
				...
				align = align || 'left',
				//svgObject = layer.getSvg(),
				fontSize = parseInt(style.fontSize || style.font),
				span,
				spanWidth,
				transformOriginX;
			...
			if (!rotation || CSStransform) {
				span = createElement('span', {
					...
					left: x + PX,
					whiteSpace: 'nowrap',
					bottom: (layer.height - y) + PX,
					color: fill,
					lineHeight: mathRound(fontSize * .75) + PX
				}), div);
				...
				var radians = (rotation || 0) * Math.PI * 2 / 360, // deg to rad
					...
					baselineCorrection = parseInt(style.fontSize) / 3 || 3;
					start = align == 'left',
					end = align == 'right',
					x1 = start ? 	x : x - length * costheta,
					x2 = end ? 		x : x + length * costheta,
					y1 = start ? 	y : y - length * sintheta,
					y2 = end ?		y : y + length * sintheta;
	...
	clear: function() {
		...
		if (this.svgObject) {
			this.div.removeChild(this.svgObject);
			this.svgObject = null;	
		}
	},
	hide: function() {
...
function Chart (options) {
	 ...
	 */
	/*this.newLayer = function (appendTo, layerWidth, layerHeight) {
		var c = document.createElement('canvas');
	    if (!appendTo) appendTo = container;
	    c.id =  'highchart-id-'+ idCounter++;
	    c.width = layerWidth || (appendTo ? appendTo.offsetWidth : chartWidth);
	    c.height = layerHeight || (appendTo ? appendTo.offsetHeight : chartHeight);
	...
	/**
	 * Create a new SVG/VML layer with methods for drawing
	 */
	/*
	function newSVGLayer () {
		/**
		 * Draw text on this layer
		 *
		 * @param {String} str The string to draw
		 * @param {Number} x Horizontal position of the text's lower left corner
		 * @param {Number} y Vertical position of the text's lower left corner
		 * @param {Object} style CSS style object
		 * @param {Number} rotation The clockwise rotation in degrees
		 * @param {String} text The anchor of the text - 'start', 'middle' or 'end'
		 * 
		 * TODO: Safari doesn't paint text
		 * /
		function text(str, x, y, style, rotation, anchor) {
			...
			if (svgObject) {
				...
				svgObject.data = 'data:image/svg+xml,'+ svg
					
				// append it again for Chrome to update
				if (isWebKit) {
					//alert (svgObject.data);
			...
			// ie
				...
				// is extrapolated to the left or right or both depending on the 
				// anchor of the text
				var radians = rotation * Math.PI * 2 / 360,
					costheta = Math.cos(radians),
					sintheta = Math.sin(radians),
					length = chartWidth, // the text is not likely to be longer than this
					start = anchor == 'start',
					end = anchor == 'end',
					x1 = start ? 	x : x - length * costheta,
					x2 = end ? 		x : x + length * costheta,
		...
		if (/msie/i.test(navigator.userAgent)) {
			...
			vmlObject = createElement(DIV, null, {
				width: chartWidth + PX,
			...
			// create an object and inject SVG into it
			svgObject = createElement('object', {
				width: chartWidth,
				height: chartHeight,
				type: 'image/svg+xml'
			}, {
	...
	*/
	...
	function zoom(selection) {
		...
		if (!selection) {
			...
			if (chart.tracker.zoomX) chart.xAxis.setExtremes(selection.xMin, selection.xMax);
			if (chart.tracker.zoomY) chart.yAxis.setExtremes(selection.yMin, selection.yMax);
		}
	...
	function Axis (chart, options, horiz) {
		...
		function render() {
			// TODO: Something in here causes blinking in Firefox
			axisLayer.clear();
			...
			if (o.title && o.title.enabled && o.title.text) {
				...
				var offAxis = (horiz ? marginTop + plotHeight : marginLeft) +
					...
					o.title.margin 
					- (isIE ? parseInt(o.title.style.fontSize) / 3 : 0); // preliminary fix for vml's centerline
	...
	var chart = this,
		container = document.getElementById(optionsChart.renderTo),
		marginTop = margin[0],
...
LineSeries.prototype = {
	init: function(chart, options) {
		...
		extend (series, {
			...
			name: options.name,
			state: '',
			visible: options.visible || typeof options.visible == 'undefined'
		});
...
var PieSeries = extendClass(LineSeries, {
	...
	translate: function() {
		...
		each (data, function(point) {
			...
			point.centerSliced = map([
				Math.cos(angle) * slicedOffset + positions[0], 
				Math.sin(angle) * slicedOffset + positions[1]
			], mathRound);
	...
	getAreaCoords: function() {
		...
		each (this.data, function(point) {
			...
			point.tooltipPos = [
				centerX + 2 * Math.cos((start + end) / 2) * radius / 3,
				centerY + 2 * Math.sin((start + end) / 2) * radius / 3
			];
